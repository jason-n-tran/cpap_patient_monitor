...
The patient name can be typed in the entry box. The CPAP pressure can be typed in as an integer between 4 and 25 inclusive in units of cmH2O. Attempts to upload invalid values will display an error pop-up box explaining whether the input was not numeric or was not in the valid range. Clicking the "Select CPAP data file" button will open a file explorer window to allow for selection of a patient's cpap pressure readings file. After file has been selected, a message box will display the pathway to the selected file to confirm before analyzing and displaying the results for breathing rate in breaths per minute, number of apnea events which will be red if it is greater than 1, and a plot of flow in cubic meters per second vs. time in seconds. Again all of this information can be sent to the server and stored in the database by clicking upload.
